...
由 Taylor 展开，
\[
u(t_0+h)=u(t_0)+h\,u'(t_0)+\frac{h^2}{2}u''(t_0)+O(h^3).
\]
这里取
\[
t_0=0,\qquad h=0.1.
...
代入展开式，得
\[
u(0.1)\approx u(0)+0.1\,u'(0)+\frac{0.1^2}{2}u''(0)
\]
\[
=1+0.1+\frac{0.01}{2}
=1.105.
...
这说明：Taylor 展开法就是在已知点附近，把解写成多项式近似，并利用微分方程计算高阶导数，从而得到更高精度的近似值。
\end{example}
